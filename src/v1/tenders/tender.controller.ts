...
L'appel d'offres sera visible par tous les membres de la plateforme qui pourront soumettre leurs offres jusqu'à la date limite de soumission.`,
    ...
    schema: {
      ...
      properties: {
        productId: {
          ...
          description: 'ID technique du produit recherché',
        },
        titre: {
          type: 'string',
          example: 'Fourniture de 100 tonnes de Maïs jaune',
          description: "Titre explicite de l'appel d'offres",
        },
        description: {
          type: 'string',
          example:
            'Nous recherchons un fournisseur pour 100 tonnes de maïs jaune de qualité supérieure...',
          description: 'Cahier des charges ou description détaillée du besoin',
        },
        quantite: {
          type: 'number',
          example: 100,
          description: 'Quantité totale souhaitée',
        },
        unite: {
          type: 'string',
          example: 'tonnes',
        ...
        dateLimite: {
          ...
          format: 'date-time',
          description:
            'Date et heure limite pour la réception des offres (Format ISO 8601)',
        },
        siteLivraison: {
          type: 'string',
          example: '64b8f0c2e1d3f2a5c6b7d8e9',
          description: 'ID du site géographique de livraison',
        },
        conditionsPaiement: {
          type: 'string',
          example: 'Paiement à la livraison après contrôle qualité',
          description: 'Modalités de règlement souhaitées',
        },
        delaiLivraisonSouhaite: {
          type: 'string',
          example: 'Maximum 15 jours après commande',
          description: 'Délai maximal de livraison acceptable',
        },
        documentPieces: {
          type: 'string',
          format: 'binary',
          description:
            'Document TDR ou pièces jointes complémentaires (PDF, images, etc.)',
        },
      },
    },
  })
  ...
  @ApiOperation({
    ...
    description:
      "Permet au lanceur de modifier son appel d'offres tant qu'il est encore ouvert.",
  })
  ...
  @ApiOperation({
    ...
    description:
      "Récupère la liste de tous les appels d'offres publics. Possibilité de filtrer par statut (OUVERT, ATTRIBUE, etc.) et de rechercher par mots-clés. Chaque appel d'offres est enrichi d'un champ \"hasBid\" indiquant si vous avez déjà soumis une offre.",
  })
  @ApiQuery({
    name: 'page',
    required: false,
  ...
  @ApiOperation({
    ...
    description:
      "Récupère uniquement les appels d'offres que vous avez personnellement lancés.",
  })
  ...
  @ApiOperation({
    ...
    description:
      'Récupère toutes les informations détaillées d\'un appel d\'offres spécifique via son ID. Inclut un champ "hasBid" indiquant si vous avez déjà soumis une offre.',
  })
  @ApiParam({
    name: 'id',
  ...
  @ApiResponse({
    status: 200,
    description: "Informations détaillées de l'appel d'offres",
  })
  ...
  @ApiOperation({
    ...
    description:
      "Permet au lanceur d'annuler un appel d'offres. Cette action n'est possible que si l'appel d'offres est encore au statut OUVERT.",
  })
  @ApiParam({ name: 'id', description: "ID de l'appel d'offres à annuler" })
  @ApiResponse({
    status: 200,
    description: "L'appel d'offres a été annulé avec succès",
  })
...
Note : Un lanceur ne peut pas soumissionner à son propre appel d'offres. L'offre doit être soumise avant la date limite.`,
  ...
  @ApiParam({
    name: 'tenderId',
    description: "ID de l'appel d'offres concerné",
  })
  @ApiBody({
    schema: {
      type: 'object',
  ...
  @ApiOperation({
    ...
    description:
      "Permet de modifier son offre tant que l'appel d'offres est encore ouvert.",
  })
  ...
  @ApiOperation({
    ...
    description:
      "Permet de retirer son offre tant que l'appel d'offres est encore ouvert.",
  })
  @ApiParam({ name: 'tenderId', description: "ID de l'appel d'offres" })
  @ApiParam({ name: 'bidId', description: 'ID de la soumission' })
  ...
  @ApiOperation({
    ...
    description:
      "Liste toutes les soumissions effectuées pour un appel d'offres donné. Les offres sont triées par prix (du moins cher au plus cher).",
  })
  @ApiParam({ name: 'tenderId', description: "ID de l'appel d'offres" })
  ...
  @ApiOperation({
    ...
    description:
      "Récupère la liste de toutes les offres (bids) que vous avez soumises sur différents appels d'offres.",
  })
  @ApiQuery({ name: 'page', required: false, type: Number, example: 1 })
  @ApiQuery({ name: 'limit', required: false, type: Number, example: 20 })
  @ApiQuery({
    ...
    required: false,
    type: String,
    description: "Rechercher par titre de l'appel d'offres ou vos observations",
  })
...
C'est à cette étape que les soumissions deviennent officiellement visibles pour le lanceur pour analyse.`,
  ...
  async openSealed(@Req() req: any, @Param('tenderId') tenderId: string) {
    const userId = req.user?.userId;
    if (!userId) throw new BadRequestException('Utilisateur non authentifié');
    const tender = await this.tenderService.ouvrirDepouillement(
      userId,
      tenderId,
    );
    return {
      status: 'success',
      message: 'Dépouillement ouvert avec succès',
      data: [tender],
    };
  }
...
L'attribution du marché entraîne automatiquement le rejet de toutes les autres soumissions non retenues.`,
  ...
  @ApiResponse({
    status: 200,
    description: 'Le marché a été attribué avec succès',
  })
  async award(
    @Req() req: any,
    @Param('tenderId') tenderId: string,
    ...
    return {
      status: 'success',
      message: 'Marché attribué avec succès',
      data: [tender],
    };
  }
}
